...
func (u *udpPacketConn) ReadFrom(buf []byte) (n int, addr net.Addr, err error) {
	for {
		...
		return copy(buf, pkt.Payload()), &net.UDPAddr{IP: header.Src, Port: int(pkt.SourcePort())}, nil
	}
}
...
func (u *udpPacketConn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	...
	buf := make([]byte, len(p)+udpHeaderLength)

	pkt := udpPacket(buf)

	pkt.SetLength(uint16(len(buf)))
...
func ListenDHCPClient(ctx context.Context, ifaceName string) (net.PacketConn, error) {
	...
	conn, err := dialer.ListenPacket(ctx, "ip4:udp", "", dialer.WithInterface(ifaceName))
	if err != nil {
		return nil, err
	}
